...
```{r}
ggplot(cleaned_data, aes(x = Points_Scored, fill = factor(SuperBowl_Winner))) + geom_histogram(position = "identity", alpha = 0.6, bins = 30) + scale_fill_manual(values = c("0" = "grey70", "1" = "darkgoldenrod2"), labels = c("Non-Winner", "Winner")) + labs(title = "Distribution of Points Scored", subtitle = "Super Bowl Winners vs. Non-Winners", x = "Points Scored", y = "Count") + theme_minimal()
```
...
ggplot(cleaned_data, aes(x = factor(SuperBowl_Winner), y = Yards, fill = factor(SuperBowl_Winner))) +
  ...
  scale_fill_manual(values = c("0" = "grey70", "1" = "darkgoldenrod2"), 
                    labels = c("Non-Winner", "Winner")) +
  labs(title = "Distribution of Total Yards",
       x = "",
       y = "Yards") +
  theme_minimal() +
